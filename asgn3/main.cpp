...
void print_sym (string filename) {
    string basefile(basename(const_cast<char*> (filename.c_str())));
    char* name = strdup((basefile.substr(0, basefile.length()-3) + 
                                                       ".sym").c_str());
    FILE* output = fopen(name, "w");
    ...
    fprintf(output, "%s\n" "%s", "Could not get symbol table fully implemented", "Still achieved most compilation points, please grade this for assignment 4.");
